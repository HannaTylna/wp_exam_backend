...
( function( $, api ) {
	...
	api.controlConstructor['armata-repeater-text'] = api.Control.extend({
...
jQuery(document).ready(function($) {
    ...
    $('#armata-img-container img').click(function() {      
        var item_id = $(this).parent().parent().parent().parent().attr('id');

        $('#' + item_id).find('li').each(function() {
            $('#' + item_id).find('img').removeClass('armata-radio-img-selected') ;
        });

        $(this).addClass('armata-radio-img-selected') ;
    });
